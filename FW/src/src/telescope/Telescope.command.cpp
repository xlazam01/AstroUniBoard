...
bool Telescope::command(char reply[], char command[], char parameter[], bool *suppressFrame, bool *numericReply, CommandError *commandError) {
  ...
  if (command[0] == 'E') {
    ...
    // :EDFU#     Jump to DFU bootloader for firmware upload
    //            Returns: Nothing (device disconnects and re-enumerates as DFU)
    if (command[1] == 'D' && parameter[0] == 'F' && parameter[1] == 'U' && parameter[2] == 0) {
