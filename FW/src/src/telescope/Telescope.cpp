...
// Focuser/Rotator activity LED (when mount is not present, repurpose MOUNT_LED_PIN)
#if STATUS_MOUNT_LED != OFF && MOUNT_LED_PIN != OFF && !defined(MOUNT_PRESENT) && (defined(FOCUSER_PRESENT) || defined(ROTATOR_PRESENT))
  void focRotActivityLed() {
    bool moving = false;
    #ifdef ROTATOR_PRESENT
      moving = axis3.isSlewing();
    #endif
    #if AXIS4_DRIVER_MODEL != OFF
      if (!moving) moving = axis4.isSlewing();
    #endif
    #if AXIS5_DRIVER_MODEL != OFF
      if (!moving) moving = axis5.isSlewing();
    #endif
    #if AXIS6_DRIVER_MODEL != OFF
      if (!moving) moving = axis6.isSlewing();
    #endif
    #if AXIS7_DRIVER_MODEL != OFF
      if (!moving) moving = axis7.isSlewing();
    #endif
    #if AXIS8_DRIVER_MODEL != OFF
      if (!moving) moving = axis8.isSlewing();
    #endif
    #if AXIS9_DRIVER_MODEL != OFF
      if (!moving) moving = axis9.isSlewing();
    #endif
    digitalWriteEx(MOUNT_LED_PIN, moving ? MOUNT_LED_ON_STATE : !MOUNT_LED_ON_STATE);
  }
#endif
...
void Telescope::init(const char *fwName, int fwMajor, int fwMinor, const char *fwPatch, int fwConfig) {
  ...
  // focuser/rotator activity LED (solid ON when any axis is moving)
  #if STATUS_MOUNT_LED != OFF && MOUNT_LED_PIN != OFF && !defined(MOUNT_PRESENT) && (defined(FOCUSER_PRESENT) || defined(ROTATOR_PRESENT))
    pinModeEx(MOUNT_LED_PIN, OUTPUT);
    digitalWriteEx(MOUNT_LED_PIN, !MOUNT_LED_ON_STATE);
    VF("MSG: Telescope, start focuser/rotator activity LED task (rate 100ms priority 4)... ");
    if (tasks.add(100, 0, true, 4, focRotActivityLed, "FRLed")) { VLF("success"); } else { VLF("FAILED!"); }
  #endif
}
