...
// search "TIMER_TONE" in github.com/stm32duino/Arduino_Core_STM32/blob/main/variants to find the timer use
  ...
  #define STM32_TIMER1       TIM17   // for STM32G0B1 (tone uses TIM6, servo TIM7)
  #define STM32_TIMER2       TIM16   // general purpose 16-bit timers
  #define STM32_TIMER3       TIM15   // TIM15/16/17 are safe, no PWM conflicts
...
bool HAL_HWTIMER_INIT(uint8_t priority, HardwareTimer * hwtimer, void (*wrapper)()) {
    ...
    uint32_t timerClk = hwtimer->getTimerClkFreq();
    uint32_t prescaleFactor = timerClk/F_COMP; // for example, 72000000/4000000 = 18
    hwtimer->setPrescaleFactor(prescaleFactor);
    hwtimer->setOverflow(4000);          // startup one millisecond

    // Debug: show timer clock and prescaler for verification
    DF("MSG: HWTimer clk="); D(timerClk); DF(" PSC="); D(prescaleFactor); DLF("");
